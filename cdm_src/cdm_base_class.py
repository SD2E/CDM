...
class CombinatorialDesignModel(metaclass=ABCMeta):
    ...
    def get_df_from_run_id(self, run_id="latest", csv_name="predicted_data") -> pd.DataFrame:
        """
        Retrieves Test Harness results (DataFrames) generated by the current class instance.
        :param run_id: run_id of the Test Harness run to get results from. Defaults to the latest run
                       that was kicked off by the current instance.
        :param csv_name: name of the csv file you want to retrieve. Defaults to "predicted_data".
        :return: desired DataFrame of Test Harness results.
        """
        if run_id == "latest":
            run_id = self.th.list_of_this_instance_run_ids[-1]

        location = os.path.join(self.output_path, "test_harness_results/runs/run_{}/{}.csv".format(run_id,
                                                                                                   csv_name))
